...
Macronutrients, except potassium, were <1000 ppm. 
...
si <- 
   best_models$silica %>% 
   emmeans(~  foliar_trt| time) %>% #compares foliar_trt within each irrigation 
   cld(alpha = 0.05, reversed = TRUE, adjust = "none", Letters=letters) %>% 
   as.data.frame() %>% 
   mutate(letter = trimws(.group)) 
...
## Suficiency ranges for leaf sap  
...
```{r}
#| tbl-cap: Sufficiency ranges, average values for macro and micro-nutrients at 40, 72, 86, and 100 days after blooming (DAB), and statuses of the mineral nutrients during fruit growth.  
#| 
sap_all_samples %>% 
  # summarise by time 
   group_by(time_num) %>% 
   summarise(across(where(is.numeric), \(x)  mean(x, na.rm= TRUE))) %>% 
   ungroup() %>% 
  # put variables in a different order 
  ...
  # change to abbreviated nutrient names 
    mutate(response = case_when(
    response == "brix"~ "°Brix (%)",
    response == "n_conv_eff" ~ "N Conv\nEff",
    response == "nitrogen" ~ "N",
...
```
Sap analysis revealed that most micronutrients are below the minimum levels during fruit growth. 
